...
/*@ axiomatic SysfsStr {
    ...
    logic bool strncmp(char* string1, char* string2, size_t n) =
       ...
       :
          (string1[0] == string2[0] ?
	     (string1[0] == '\0' ? (bool)true : strncmp(string1 + 1, string2 + 1, (size_t)(n - 1)))
          :
	     (bool)false);
    ...
    lemma strncmp_definition:
       \forall char* string1, char* string2, size_t n;
          valid_str(string1) &&
          valid_str(string2) ==>
          (\forall size_t i; 0 <= i < n ==> string1[i] == string2[i]) ==>
          strncmp(string1, string2, n);
    ...
    lemma sysfs_strlen_n_equal:
       \forall char* string1, char* string2, size_t n;
	     ...
	     (string1[i] != string2[i])) ==>
          strncmp(string1, string2, (size_t)\min(sysfs_strlen(string1), sysfs_strlen(string2))) == (bool)false;

    lemma sysfs_strlen_bsn:
       \forall char* string, size_t n;
          valid_str(string) &&
          (\forall size_t i; 0 <= i < n ==> string[i] != '\0') &&
          string[n] == '\n' &&
          string[n + 1] == '\0' ==>
    ...
    lemma sysfs_strlen_equal:
	  ...
	  (sysfs_strlen(string1) == sysfs_strlen(string2) &&
           strncmp(string1, string2, (size_t)\min(sysfs_strlen(string1), sysfs_strlen(string2))) == (bool)true);
    }
 */
